...
tauto. tauto. tauto. tauto. 
   generalize (upb_pos m z H0). fold p. intro. omega. 
Qed.
...
fold (Shift m Md.kd x) in |- *.
  set (cASm := cA (Shift m Md.kd x) Md.kd) in |- *.
  set (cAm := cA m Md.kd) in |- *.
  simpl in |- *.
  fold (Shift m Md.kd x) in IHi.
  fold cASm in IHi.
  fold cAm in IHi.
  rewrite IHi in |- *.
  unfold cASm in |- *.
...
induction i.
 simpl in |- *.
    tauto.
fold (Shift m k x) in |- *.
  set (cFS := cF (Shift m k x)) in |- *.
  set (cFm := cF m) in |- *.
  simpl in |- *.
  fold cFm in IHi.
  fold (Shift m k x) in IHi.
  fold cFS in IHi.
  rewrite IHi in |- *.
  unfold cFS in |- *.
  rewrite cF_Shift in |- *.
